...
class pl_DGA(LightningModule):
    ...
    def inference(self, g, device, batch_size, num_workers, buffer_device=None):
        ...
        print(f"开始推理，共 {len(dataloader)} 个batch...")
        ...
        for batch_idx, (input_nodes, output_nodes, blocks) in enumerate(tqdm(dataloader)):
            ...
            if torch.cuda.is_available():
                gpu_mem_before = torch.cuda.memory_allocated() / 1024 ** 2  # 转换为MB
            ...
            if torch.cuda.is_available():
                ...
                gpu_mem_after = torch.cuda.memory_allocated() / 1024 ** 2  # 转换为MB
            ...
            if (batch_idx + 1) % 10 == 0:
                ...
                if torch.cuda.is_available() and len(gpu_memory_usage) >= 10:
                    avg_gpu = np.mean(gpu_memory_usage[-10:])
                    print(f"  Batch {batch_idx+1}: 推理时间 {batch_inference_time:.4f}s, "
                          f"平均 {avg_time:.4f}s, GPU内存 {gpu_mem_after:.2f}MB")
                else:
                    print(f"  Batch {batch_idx+1}: 推理时间 {batch_inference_time:.4f}s, 平均 {avg_time:.4f}s")
        ...
        print(f"平均每个batch推理时间: {np.mean(inference_times):.4f} ± {np.std(inference_times):.4f} 秒")
        print(f"最小batch推理时间: {np.min(inference_times):.4f} 秒")
        print(f"最大batch推理时间: {np.max(inference_times):.4f} 秒")
        
        if torch.cuda.is_available() and gpu_memory_usage:
            print(f"\nGPU内存使用统计:")
            print(f"平均GPU内存使用: {np.mean(gpu_memory_usage):.2f} ± {np.std(gpu_memory_usage):.2f} MB")
            print(f"最小GPU内存使用: {np.min(gpu_memory_usage):.2f} MB")
            print(f"最大GPU内存使用: {np.max(gpu_memory_usage):.2f} MB")
            
            # 获取峰值内存使用
            peak_memory_mb = torch.cuda.max_memory_allocated() / 1024 ** 2
            print(f"峰值GPU内存使用: {peak_memory_mb:.2f} MB")
        
        print(f"处理节点总数: {g.num_nodes()}")
        print(f"每秒处理节点数: {g.num_nodes() / total_inference_time:.2f} nodes/s")
        print("="*60 + "\n")
...
def run(args: DictConfig):
    ...
    with torch.no_grad():
        ...
        y_hat = model.inference(graph, device, args.bs, 0, "cuda")
        ...
        print("整体推理统计")
        print("="*60)
        print(f"模型加载后总推理时间: {total_inference_end - total_inference_start:.4f} 秒")
        if torch.cuda.is_available():
            total_peak_memory_mb = torch.cuda.max_memory_allocated() / 1024 ** 2  # MB
            print(f"整个推理过程峰值GPU内存: {total_peak_memory_mb:.2f} MB")
        print("="*60 + "\n")
