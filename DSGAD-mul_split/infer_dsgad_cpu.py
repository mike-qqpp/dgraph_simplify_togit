# The file for test inference - CPU ONLY VERSION
# 强制使用CPU，移除所有GPU相关代码
...
def batch_inference(model, g, features, labels, test_mask, threshold, batch_size, device):
    ...
    return test_probs, test_labels, test_y_pred, inference_time, 0.0  # 返回0.0作为内存占位符
...
def evaluate_test_set(model, g, features, labels, test_mask, threshold, batch_size, device):
    ...
    # 批处理推理
    test_probs, test_labels, test_y_pred, inference_time, _ = batch_inference(
    ...
    # 计算测试集指标
    try:
        test_auc = skmetrics.roc_auc_score(test_labels.numpy(), test_probs.numpy())
        test_ap = skmetrics.average_precision_score(test_labels.numpy(), test_probs.numpy())
        test_recall = skmetrics.recall_score(test_labels.numpy(), test_y_pred.numpy())
        test_precision = skmetrics.precision_score(test_labels.numpy(), test_y_pred.numpy())
        test_f1_macro = skmetrics.f1_score(test_labels.numpy(), test_y_pred.numpy(), average='macro')
        test_f1_micro = skmetrics.f1_score(test_labels.numpy(), test_y_pred.numpy(), average='micro')
        test_accuracy = skmetrics.accuracy_score(test_labels.numpy(), test_y_pred.numpy())
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DSGAD Test Inference (CPU Only)')
    parser.add_argument('--dataset', type=str, default='yelp', help='dataset')
    parser.add_argument('--device', type=str, default='cpu', help='device: cpu only (this parameter is ignored)')
    ...
    model, loaded_checkpoint = load_model(
        ...
        model_config={},  # 可以从checkpoint获取配置
        device=device
    )
    ...
    print(f"\nCPU内存状态: 未测量峰值内存，请使用外部工具如/usr/bin/time")
    ...
    print("\n注意: 此CPU版本未测量内存使用，请使用外部工具进行内存测量")
    print("="*60)
    ...
    # 保存结果到文件
    with open('test_result_cpu.txt', 'a+') as f:
        f.write(f'{result}\n')
    
    # 保存预测结果
    if args.save_predictions:
        # 确保保存路径的目录存在
        pred_dir = os.path.dirname(args.predictions_path)
        if pred_dir:
