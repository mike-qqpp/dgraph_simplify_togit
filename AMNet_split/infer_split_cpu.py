...
推理脚本 - CPU版本
使用方法:
python infer_split_cpu.py --data_path <数据文件> --model_path <模型文件> --output_dir <输出目录>
"""
...
def batch_inference(model, data, mask, batch_size=512):
    """
    批处理推理（只推理mask指定的节点）
    返回: (输出, 推理的节点索引)
    """
    ...
    return final_output, masked_indices
...
def main(args):
    ...
    print("\n[阶段2] 加载模型...")
    try:
        model = load_model(args.model_path, device, in_channels=data.x.shape[1])
    except Exception as e:
        print(f"加载模型时出错: {e}")
        return
    ...
    outputs, indices = batch_inference(
        model, data, data.test_mask, batch_size=args.batch_size
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='AMNet 图异常检测推理 (CPU版本)')
