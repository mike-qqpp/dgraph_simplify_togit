...
使用方法:
    python dataset_processor.py --dataset dgraphfin --data_path ../../data --output_dir ../data
    python dataset_processor.py --dataset mydataset --n_1hop_samples 20 --n_2hop_samples 40
"""
...
def create_dataset(data_path='./datasets/dgraphfin', output_dir='../data', 
    ...
    # 加载原始数据集数据
    npz_path = os.path.join(data_path, f'{dataset_name}.npz')
    ...
    # 保存邻接列表
    adjlist_path = os.path.join(output_dir, f"{dataset_name}_homo_adjlists.pickle")
    ...
    # ========================================
    print(f"\n步骤2: 保存DGL图...")
    graph_path = os.path.join(output_dir, f"graph-{dataset_name}.bin")
    ...
    # 保存邻居特征
    output_path = os.path.join(output_dir, f"{dataset_name}_neigh_feat.csv")
    ...
    mask_path = os.path.join(output_dir, f"{dataset_name}_masks.npz")
    ...
    # ========================================
    print(f"\n步骤5: 创建包含所有信息的.npz文件...")
    full_npz_path = os.path.join(output_dir, f"{dataset_name}_full.npz")
...
def matrix_powers_gpu_optimized_v2(adj_list, n, block_size, matrix_prefix):
    ...
    if has_cuda:
        ...
        if dense_memory > gpu_memory * 0.1:
...
def process_matrix_powers(output_dir='../data', dataset_name='dgraphfin'):
    """处理数据集的矩阵幂计算
    ...
    """
    print("\n" + "="*60)
    print(f"开始处理{dataset_name}矩阵幂 (1-10阶)")
    print("="*60)
    ...
    matrix_prefix = os.path.join(output_dir, f"{dataset_name}_adj_power_matrix")
...
def main(dataset_name='dgraphfin', data_path='../../data', output_dir='../data',
         n_1hop_samples=10, n_2hop_samples=20, batch_size=10000):
    """主函数
    ...
    """
    print("="*60)
    print(f"{dataset_name}数据集处理流程 - 优化版")
    print("="*60)
    ...
    try:
        ...
        if not torch.cuda.is_available():
            ...
            if response.lower() != 'y':
                ...
                process_matrix_powers(output_dir, dataset_name)
        else:
            process_matrix_powers(output_dir, dataset_name)
        ...
        print(f"\n数据文件保存在: {output_dir}/")
        print(f"包含以下文件:")
        print(f"  1. graph-{dataset_name}.bin - DGL图文件")
        print(f"  2. {dataset_name}_homo_adjlists.pickle - 邻接列表")
        print(f"  3. {dataset_name}_neigh_feat.csv - 邻居特征")
        print(f"  4. {dataset_name}_masks.npz - 训练/验证/测试掩码")
        print(f"  5. {dataset_name}_full.npz - 完整数据集")
        
        # 检查矩阵幂文件
        matrix_files = []
        for k in range(1, 11):
            matrix_file = os.path.join(output_dir, f"{dataset_name}_adj_power_matrix{k}.pkl")
            if os.path.exists(matrix_file):
                file_size = os.path.getsize(matrix_file) / (1024**3)
                matrix_files.append(f"  {k}. {dataset_name}_adj_power_matrix{k}.pkl - {k}阶矩阵幂 ({file_size:.2f} GB)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='数据集处理工具 - 支持灵活的数据集名称参数',
...
使用示例:
    ...
    # 处理其他数据集
    python dataset_processor.py --dataset mydataset --data_path ./data --output_dir ./output
    
    # 完整参数示例
    python dataset_processor.py --dataset amazon --data_path ../../data --output_dir ../processed --n_1hop_samples 20 --n_2hop_samples 40 --batch_size 5000
        """
    )
    ...
    parser.add_argument('--output_dir', type=str, default='../data',
                        help='输出目录 (默认: ../data)')
    parser.add_argument('--n_1hop_samples', type=int, default=10,
                        help='1-hop邻居采样数 (默认: 10)')
    parser.add_argument('--n_2hop_samples', type=int, default=20,
                        help='2-hop邻居采样数 (默认: 20)')
    ...
    main(
        ...
        n_1hop_samples=args.n_1hop_samples,
        n_2hop_samples=args.n_2hop_samples,
