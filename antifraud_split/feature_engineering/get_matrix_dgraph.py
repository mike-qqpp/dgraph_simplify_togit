...
########get_matrix_dgraphfin.py
"""
DGraphFin数据集的矩阵幂计算脚本
...
def matrix_powers_dgraphfin(adj_list, n, matrix_prefix):
    """
    为DGraphFin数据集计算1-10阶矩阵幂
...
def check_output_files(DATADIR):
    """
    检查并显示已生成的矩阵幂文件
    ...
    for k in range(1, 11):
        # 检查.pkl文件
        pkl_file = os.path.join(DATADIR, f"dgraphfin_adj_power_matrix{k}.pkl")
        npz_file = os.path.join(DATADIR, f"dgraphfin_adj_power_matrix{k}.npz")
        
        if os.path.exists(pkl_file):
            file_size = os.path.getsize(pkl_file) / (1024**3)
            matrix_files.append(f"  dgraphfin_adj_power_matrix{k}.pkl - {file_size:.3f} GB")
        elif os.path.exists(npz_file):
            file_size = os.path.getsize(npz_file) / (1024**2)
            matrix_files.append(f"  dgraphfin_adj_power_matrix{k}.npz - {file_size:.2f} MB")
...
def main():
    ...
    # DGraphFin邻接列表路径
    adjlist_filename = "dgraphfin_homo_adjlists.pickle"
    ...
    if not os.path.exists(adjlist_path):
        ...
        print(f"请先运行data_process_dgraphfin.py生成邻接列表")
    ...
    # 设置输出文件前缀
    matrix_prefix = os.path.join(DATADIR, "dgraphfin_adj_power_matrix")
    ...
    try:
        matrix_powers_dgraphfin(adj_list, n, matrix_prefix)
    ...
    # 检查输出文件
    check_output_files(DATADIR)
    
    print("\n" + "=" * 60)
    print("DGraphFin矩阵幂计算完成!")
    print("=" * 60)
    
    print(f"\n总用时: {total_time:.2f}秒 ({total_time/60:.2f}分钟)")
    print(f"输出文件保存在: {DATADIR}")
    print("\n生成的文件格式:")
    print("  - dgraphfin_adj_power_matrix1.pkl/npz: 1阶邻接矩阵")
    print("  - dgraphfin_adj_power_matrix2.pkl/npz: 2阶邻接矩阵幂")
    print("  - ...")
    print("  - dgraphfin_adj_power_matrix10.pkl/npz: 10阶邻接矩阵幂")
