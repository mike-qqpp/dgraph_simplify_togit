...
def rgtan_main(feat_df, graph, train_idx, test_idx, labels, args, cat_features, neigh_features: pd.DataFrame, nei_att_head):
    ...
    for fold, (trn_idx, val_idx) in enumerate(kfold.split(feat_df.iloc[train_idx], y_target)):
        ...
        for epoch in range(start_epoch, args['max_epochs']):
            ...
            for step, (input_nodes, seeds, blocks) in enumerate(train_pbar):
                ...
                if step == 0 and epoch == 0:
                    print(f"\nDEBUG: First batch - Label analysis")
                    print(f"batch_labels shape: {batch_labels.shape}")
                    print(f"batch_labels unique values: {torch.unique(batch_labels)}")
                    print(f"batch_labels distribution: {torch.bincount(batch_labels)}")
                    print(f"0/1 count: {(batch_labels == 0).sum().item()} zeros, {(batch_labels == 1).sum().item()} ones")
                    print(f"train_batch_logits shape: {train_batch_logits.shape}")
                    print(f"train_batch_logits range: [{train_batch_logits.min():.4f}, {train_batch_logits.max():.4f}]")
                ...
                if step == 0 and epoch == 0:
                    print(f"seeds_valid_mask sum: {seeds_valid_mask.sum().item()}/{len(seeds_valid_mask)}")
                    if seeds_valid_mask.any():
                        print(f"Valid labels found: {batch_labels[seeds_valid_mask].unique()}")
                ...
                if seeds_valid_mask.any():
                    ...
                    # 调试：检查损失计算前的值
                    if step == 0 and epoch == 0:
                        print(f"\nDEBUG: Loss calculation info")
                        print(f"train_batch_logits_valid shape: {train_batch_logits_valid.shape}")
                        print(f"batch_labels_valid shape: {batch_labels_valid.shape}")
                        print(f"Sample logits: {train_batch_logits_valid[:3]}")
                        print(f"Sample labels: {batch_labels_valid[:3]}")
                    
                    train_loss = loss_fn(train_batch_logits_valid, batch_labels_valid)
                    
                    # 调试：检查损失值
                    if step == 0 and epoch == 0:
                        print(f"Initial loss: {train_loss.item():.6f}")
                        print(f"Logits mean: {train_batch_logits_valid.mean().item():.6f}")
                        print(f"Logits std: {train_batch_logits_valid.std().item():.6f}")
                    ...
                    if step == 0 and epoch == 0:
                        ...
                        for name, param in model.named_parameters():
                            if param.grad is not None:
                                ...
                                if grad_norm > 0:
                                    non_zero_grads += 1
                                    if grad_norm > 0.001:  # 只打印较大的梯度
                                        print(f"  {name}: grad norm = {grad_norm:.6f}")
                        print(f"Total gradient norm: {total_grad_norm:.6f}")
                        print(f"Parameters with non-zero gradients: {non_zero_grads}/{sum(1 for _ in model.parameters())}")
                    ...
                    with torch.no_grad():
                        ...
                        try:
                            if len(torch.unique(batch_labels_valid)) > 1:  # 确保有正负样本
                            ...
                            train_ap_list.append(0.5)
                            if step == 0 and epoch == 0:
                                print(f"AUC/AP calculation error: {e}")
        ...
        with torch.no_grad():
            test_pbar = tqdm(test_dataloader, desc='Testing', bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}')
            
            for step, (input_nodes, seeds, blocks) in enumerate(test_pbar):
                batch_inputs, batch_work_inputs, batch_neighstat_inputs, batch_labels, lpa_labels = load_lpa_subtensor(num_feat, cat_feat, nei_feat, neigh_padding_dict, labels,
                                                                                                                       seeds, input_nodes, device, blocks)
                ...
                test_pbar.set_postfix({'batches': step+1})
    ...
    # 计算验证集指标 - 使用所有fold的oof_predictions
    ...
    # 计算OOF指标 - 修复：labels现在是Tensor，不是DataFrame/Series
    if isinstance(labels, torch.Tensor):
    ...
    # 修复：labels现在是Tensor，不是DataFrame/Series
    if isinstance(labels, torch.Tensor):
...
def loda_rgtan_data(dataset: str, test_size: float):
    ...
    if dataset in ['dgraphfin', 'dgraph_exp130_nona']:
