...
class TransEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if in_feats_dim % att_head_num == 0:
            self.att_head_size = in_feats_dim // att_head_num
        else:
            self.att_head_size = (in_feats_dim // att_head_num) + 1
            print(f"Warning: in_feats_dim ({in_feats_dim}) is not divisible by att_head_num ({att_head_num}). "
                  f"Adjusted att_head_size to {self.att_head_size}, total head size will be {self.att_head_size * att_head_num}")
    ...
    def transpose_for_scores(self, input_tensor):
        ...
        if feature_dim % self.att_head_num == 0:
            new_x_shape = input_shape[:-1] + (self.att_head_num, self.att_head_size)
            input_tensor = input_tensor.view(*new_x_shape)
        else:
            target_dim = self.att_head_size * self.att_head_num
            if feature_dim < target_dim:
                padding_size = target_dim - feature_dim
                padding = torch.zeros(*input_shape[:-1], padding_size, dtype=input_tensor.dtype, device=input_tensor.device)
    ...
    def forward_neigh_emb(self, neighstat_feat):
        ...
        context_shape = context_layer.size()
        batch_size = context_shape[0]
        context_layer = context_layer.reshape(batch_size, -1)
        
        if context_layer.size(-1) > self.total_head_size:
            context_layer = context_layer[:, :self.total_head_size]
        elif context_layer.size(-1) < self.total_head_size:
            padding = torch.zeros(batch_size, self.total_head_size - context_layer.size(-1), 
                                  dtype=context_layer.dtype, device=context_layer.device)
            context_layer = torch.cat([context_layer, padding], dim=-1)
    ...
    def forward(self, cat_feat: dict, neighstat_feat: dict):
        ...
        nei_output = None
        if neighstat_feat is not None and len(neighstat_feat) > 0:
...
class RGTAN(nn.Module):
    def __init__(self,
        ...
        self.n_classes = 2  # 强制设置为2，因为只处理标签0和1
        self.heads = heads
        self.activation = activation
        ...
        self.layers.append(nn.Embedding(
            self.n_classes + 1, self.in_feats + self.nei_feat_dim, padding_idx=self.n_classes))
        ...
        self.layers.append(nn.Sequential(
            ...
            nn.Linear(self.hidden_dim * self.heads[0], self.in_feats + self.nei_feat_dim)
        ))
    ...
    def forward(self, blocks, features, labels, n2v_feat=None, neighstat_feat=None):
        ...
        # 将非0/1的标签映射到padding_idx (n_classes=2)
        labels_processed = labels.clone()
        labels_processed[~valid_mask] = self.n_classes  # 使用padding_idx=2
        ...
        if n2v_feat is None and neighstat_feat is None:
            ...
            if isinstance(cat_h, torch.Tensor):
                h = features + cat_h
            else:
                h = features
            
            if isinstance(nei_h, torch.Tensor) and nei_h.numel() > 0:
                nei_h_unsqueezed = nei_h.unsqueeze(1)
                h = torch.cat([h, nei_h_unsqueezed], dim=-1)
        
        label_embed = self.input_drop(self.layers[0](labels_processed))
        
        part1 = self.layers[1](h)
        part2 = self.layers[2](label_embed)
        
        label_embed = part1 + part2
