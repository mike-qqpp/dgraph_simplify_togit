...
def load_data(data, k=2, prefix=''):
    ...
    if data == 'yelp':
        ...
        # 加载DGraphFin数据集
        print(f"加载DGraphFin数据集...")
        
        # 加载特征和标签
        data_file = np.load(os.path.join(prefix, 'dgraphfin_full.npz'))
        feat_data = data_file['x']
        labels = data_file['y']
        
        print(f"  节点数: {feat_data.shape[0]}, 特征维度: {feat_data.shape[1]}")
        
        # 加载原始边索引（从原始dgraphfin.npz）
        try:
            orig_file = np.load(os.path.join(prefix, 'dgraphfin.npz'))
        ...
        # 加载邻接列表并转换为边索引
        if os.path.exists(pickle_file['dgraphfin']):
            with open(pickle_file['dgraphfin'], 'rb') as file:
                relation1 = pickle.load(file)
            file.close()
            relation1 = dict_to_edge_index(relation1)
        ...
        for i in range(1, k+1):
            try:
                # 尝试加载.npz文件（稀疏矩阵格式）
                file_name = '{}{}.npz'.format(matrix_prefix['dgraphfin'], i)
                if os.path.exists(file_name):
                    ...
                    # 尝试加载.pkl文件
                    file_name = '{}{}.pkl'.format(matrix_prefix['dgraphfin'], i)
                    if os.path.exists(file_name):
                        with open(file_name, 'rb') as file:
        ...
        raise ValueError(f"不支持的数据集: {data}")
