...
def main(args):
    if args['method'] == 'mcnn':
        ...
        from methods.rgtan.rgtan_main_dgraph_gpu import rgtan_main, loda_rgtan_data
        # 查看args['dataset']的值是什么
        print(f"Dataset value: {args['dataset']}")
        print(f"Available keys in nei_att_heads: {list(args['nei_att_heads'].keys())}")
        ...
        from methods.hogrl.hogrl_main_dgraph_v2 import hogrl_main
        hogrl_main(args)
    else:
        raise NotImplementedError("Unsupported method. ")
